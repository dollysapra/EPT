...
def main():
    ...
    # Paths to LMDBs
    training_lmdb_path = os.path.join(os.path.expanduser('~'), 'caffe2_notebooks', 'tutorial_data', 'cifar10', 'shuffled_lmdb','shuffle'+str(r))
    ...
    # Training params
    training_iters = 101        # total training iterations
    training_net_batch_size = 51  # batch size for training
    validation_images = 6000       # total number of validation images
    validation_batch = 600     # validate every <validation_interval> training iterations
    
    arg_scope = {"order": "NHWC"}
   
    lr = 0.0002 * (1. / (1. + 0.002 * iterations))
    ...
    with core.DeviceScope(device_option): 
        xent = train_model.LabelCrossEntropy(['softmax_output', label ], 'xent')
        loss = train_model.AveragedLoss(xent, "loss")
        train_model.AddGradientOperators([loss])   
        optimizer.build_sgd(train_model,base_learning_rate= lr,
                policy="fixed",momentum=0.9,
                weight_decay=0.0001)
    ...
    with core.DeviceScope(device_option):
        accuracy = brew.accuracy(val_model, ['softmax_output', label ], "Accuracy") 
