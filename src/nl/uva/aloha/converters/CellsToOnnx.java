...
import nl.uva.aloha.cellGenetic.Cell;
import nl.uva.aloha.cellGenetic.OnnxNodeInfoHolder;
import nl.uva.aloha.cellGenetic.OnnxNodeInfoHolder.OnnxOperatorTypes;
import onnx.ONNX;
...
public class CellsToOnnx 
{
	...
	public CellsToOnnx(Cell cell)
	{
		_cell = cell;
	}
	
	public CellsToOnnx(ArrayList<OnnxNodeInfoHolder> onnxOps)
	{
		_onnxOps = onnxOps;
	}
	
	public CellsToOnnx(ArrayList<OnnxNodeInfoHolder> onnxOps, Boolean processConvs)
	{
		_onnxOps = onnxOps;
	...
	public ModelProto convertToONNXModel()
	{
		...
		ValueInfoProto valueProto = GeneToOnnx.createInputProto("input_data", Tensor.reverse(INPUT_DATA_CIFAR_SHAPE));
		_graphBuilder.addInput(valueProto);
	
		ValueInfoProto valueProto_o = GeneToOnnx.createInputProto("softmax_output", Tensor.reverse(OUTPUT_DATA_CIFAR_SHAPE));
...
//		_graphBuilder.addOutput(valueProto4_o);
	...
	public NodeProto OpToOnnxNode(OnnxNodeInfoHolder op)
	{
		...
		if(op.OpType== OnnxOperatorTypes.CONVOLUTION)
		{
			...
			_nodeBuilder.addAttribute(GeneToOnnx.padsToAttributes(op.pads));
			_nodeBuilder.addAttribute(GeneToOnnx.kernelShapeToAttributes(op.kernel));
			_nodeBuilder.addAttribute(GeneToOnnx.strideToAttributes( op.stride));
			_nodeBuilder.addAttribute(GeneToOnnx.groupToAttributes(op.group));
			_nodeBuilder.addAttribute(GeneToOnnx.dilationShapeToAttributes(op.dilations));
			
			_nodeBuilder.addInput(op.name + "_weight");
			_nodeBuilder.addInput(op.name + "_bias");
			
			_graphBuilder.addInput(GeneToOnnx.createInputProto( op.name + "_weight", op.weightFormat));
			_graphBuilder.addInput(GeneToOnnx.createInputProto(op.name + "_bias", op.biasFormat) );
			
			_graphBuilder.addInitializer(GeneToOnnx.createHeWeights(op.name + "_weight",  op.weightFormat));
			_graphBuilder.addInitializer(GeneToOnnx.createHeWeights(op.name + "_bias",  op.biasFormat ));
		...
		else if(op.OpType== OnnxOperatorTypes.CONCAT)
		{
			...
			_nodeBuilder.addAttribute(GeneToOnnx.intValueToAttribute("axis", 1));
		}
		else if(op.OpType == OnnxOperatorTypes.BATCHNORM)
		{
			_nodeBuilder.setOpType("BatchNormalization");
			
			_nodeBuilder.addAttribute(GeneToOnnx.floatValueToAttribute("epsilon", (float)0.00001));
			_nodeBuilder.addAttribute(GeneToOnnx.floatValueToAttribute("momentum", (float)0.9));
			...
			_graphBuilder.addInput(GeneToOnnx.createInputProto(op.name + "_gamma", op.biasFormat));
			_graphBuilder.addInput(GeneToOnnx.createInputProto(op.name + "_beta", op.biasFormat));
			_graphBuilder.addInput(GeneToOnnx.createInputProto(op.name + "_mean", op.biasFormat));
			_graphBuilder.addInput(GeneToOnnx.createInputProto(op.name + "_var", op.biasFormat));
		   
			
			_graphBuilder.addInitializer(GeneToOnnx.createRandomWeight(op.name + "_gamma", op.biasFormat));
			_graphBuilder.addInitializer(GeneToOnnx.createRandomWeight(op.name + "_beta", op.biasFormat));
			_graphBuilder.addInitializer(GeneToOnnx.createZeroWeights(op.name + "_mean", op.biasFormat));
			_graphBuilder.addInitializer(GeneToOnnx.createOneValuedWeights(op.name + "_var", op.biasFormat));
		...
		else if(op.OpType == OnnxOperatorTypes.FC)
		{
			...
			_nodeBuilder.addAttribute(GeneToOnnx.intValueToAttribute("transB", 1));
