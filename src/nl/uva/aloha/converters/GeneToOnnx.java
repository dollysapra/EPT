...
public class GeneToOnnx {
	...
	public ModelProto convertToONNXModel()
	{
		...
		while(itr.hasNext()) 
		{
			...
			{
				Tensor outputFormat = new Tensor(1);
				
				if(Configs.DATASET.equals("CIFAR"))
					outputFormat = new Tensor(10);
				else if(Configs.DATASET.equals("PAMAP2"))
					outputFormat = new Tensor(12);
				else if (Configs.DATASET.equals("VOC"))
					outputFormat = new Tensor(20);
				outputFormat.addDimension(l.getNeuronsNum());
				
				String outputname = l.getInputConnections().get(0).getSrc().getName();// + "_" + l.getName();
				ValueInfoProto valueProto = createInputProto(outputname, Tensor.reverse(outputFormat));
				
				if(Configs.DATASET.equals("TINYYOLO"))
				{
					
					//long[] transposeValues = {0,2,3,1}; Dont know where to set them in Transpose() Layer. 
					l = new Layer("transpose", new Transpose(),1);
					
					layerOutputs.add("transpose_output" );
					_graphBuilder.addNode(layerToOnnxNode(l,layerInputs,layerOutputs));
					
					
					long[] reshapeValues = {1,12,12,4,25};
					layerInputs.clear();
					layerInputs.add(layerOutputs.get(0)); //from previous layer
					layerInputs.add("shape");
					
					_graphBuilder.addInput(createReshapeProto("shape", new Tensor(5)));
					_graphBuilder.addInitializer(createReshapeInitializer("shape", new Tensor(5), reshapeValues));
					
					l = new Layer("reshape", new Reshape(),1);
					layerOutputs.clear();
					layerOutputs.add("reshape_output");
					_graphBuilder.addNode(layerToOnnxNode(l,layerInputs,layerOutputs));
					
					valueProto = createInputProto(layerOutputs.get(0), new Tensor(1,12,12,4,25));
				}
			...
			else if(currentNeuron instanceof Convolution)
			{
				...
				if(Configs.BatchNormAfterConv) 
				{
					...
					l.setName(intermediator);
				}
				String weightName = l.getName()+"_weights";
				String biasName = l.getName()+"_bias";
				...
				if(Configs.DATASET.equals("PAMAP2"))
					...
					_graphBuilder.addInitializer(createHeWeights(weightName, weightFormat)); //TODO:CONFIG
				_graphBuilder.addInitializer(createZeroWeights(biasName, new Tensor(l.getNeuronsNum())) );
				
				if(Configs.BatchNormAfterConv) //&& (l.getOutputConnections().get(0).getDest().getNeuron() instanceof NonLinear))
				{
					ArrayList<String> intermediateInputOutput = new ArrayList<String>(); 
					
					//String BNname = l.getName();// + "BN";
					
					intermediateInputOutput.add(l.getName());
			...
			else if(currentNeuron instanceof DenseBlock)
			{
				...
				if(l.getInputFormat().getDimensionality() > 1)
				{
					NodeProto.Builder BNBuilder = NodeProto.newBuilder();
					BNBuilder.setOpType("Flatten");
					BNBuilder.setName("flat");
					BNBuilder.addInput(layerInputs.get(0));
					BNBuilder.addOutput("flat");
					_graphBuilder.addNode(BNBuilder.build());
					
					layerInputs.set(0, "flat");
				}
			...
			else if(currentNeuron instanceof Reshape)
			{
				
				long[] reshapeValues = {1,12,12,5,25};
				layerInputs.add("shape");
			...
			else if(currentNeuron.getName().contains("GLOBAL"))
			{
				ArrayList<String> intermediateInputOutput = new ArrayList<String>(); 
				String intermediator = l.getName()+"interglobalpool";
				String Flattenname = l.getName() + "Flat";
				intermediateInputOutput.add(intermediator);
				
				_graphBuilder.addNode(layerToOnnxNode(l,layerInputs,intermediateInputOutput));
				
				NodeProto.Builder FlatBuilder = NodeProto.newBuilder();
				FlatBuilder.setOpType("Flatten");
				FlatBuilder.setName(Flattenname );
				FlatBuilder.addInput(intermediator);
				
				FlatBuilder.addOutput(layerOutputs.get(0));
				_graphBuilder.addNode(FlatBuilder.build());
			}
	...
	public static NodeProto layerToOnnxNode(Layer l, ArrayList<String> inputs, ArrayList<String> outputs)
	{
		...
		{
			_nodeBuilder.setOpType("Reshape");
		}
		
		else if(currentNeuron instanceof Transpose)
		{
			_nodeBuilder.setOpType("Transpose");
			_nodeBuilder.addAttribute(permToAttributes(new int[] {0,2,3,1}));
		}
		...
		else if(currentNeuron instanceof NonLinear)
		{
			...
			else if(currentNeuron.getName().contains(NonLinearType.LeakyReLu.toString()))
			{
				...
				_nodeBuilder.addAttribute(floatValueToAttribute("alpha", new Float(0.1)));
			}
	...
	public static ValueInfoProto createReshapeProto(String name, Tensor inputTensorDims)
	{
		ValueInfoProto.Builder inputBuilder = ValueInfoProto.newBuilder();
		inputBuilder.setName(name);
		...
		TypeProto.Tensor.Builder tensorBuilder =  TypeProto.Tensor.newBuilder();
		tensorBuilder.setElemType(DataType.INT64);
		tensorBuilder.setShape(shapeBuilder.build());
	...
	public static TensorProto createReshapeInitializer(String name, Tensor inputTensorDims, long[] reshapeDims)
	{
		...
		_builder.setDataType(DataType.INT64);	
		
		ByteBuffer bb = ByteBuffer.allocate(totalValues*8);
		
		bb.order(ByteOrder.LITTLE_ENDIAN);
		for(int j=0; j< totalValues; j++)
			bb.putLong((long)reshapeDims[j]);
		bb.rewind();
	...
	public static TensorProto createpositiveWeight(String name, Tensor inputTensorDims)
	{
		TensorProto.Builder _builder = TensorProto.newBuilder();
		...
		for(int j=0; j< totalValues; j++)
		{
			bb.putFloat(j*4, new Float(Math.random()));
		}
		_builder.setRawData(ByteString.copyFrom(bb, bb.capacity()));
	...
	public static  AttributeProto permToAttributes(int[] perm)
	{
		AttributeProto.Builder _attBuilder = AttributeProto.newBuilder();
		_attBuilder.setName("perm");
		_attBuilder.setType(AttributeType.INTS);
		
		if(perm== null)
		{
			perm = new int[] {0,2,3,1};
		}
		
		for(int i=0;i<perm.length;i++)
           _attBuilder.addInts(perm[i]);
